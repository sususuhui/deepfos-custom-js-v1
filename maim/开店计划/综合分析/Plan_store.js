...
const setPageTransferPov = () => {
  ...
  if (!_.isUndefined(paramObj.Custom_params)) {
    ...
    $(".dataSheetCon>div:eq(0)").find("select[aname=Entity] option").attr("value", pov.Entity);
    $(".dataSheetCon>div:eq(0)").find("select[aname=Entity] option").trigger("change");
    // debugger
...
$("#refreshBoard").before(
  '<a  class="breadcrumb-elements-item cursor-pointer mr-2 ml-2 mcdPythonButton" onclick="parent.newStore()"><i class="icon-insert-template icon text-default mr-2"></i>生成预算门店清单</a>'
); // 新建按钮
...
function newStore() {
  ...
  cfs.request.python.web("plan_store_list", { Year: Year, Scenario: Scenario, Version: Version, b_model: b_model });
  $("#refreshBoard").click();
  $.unblockUI();
}
var cfs = {
