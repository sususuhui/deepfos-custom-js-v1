$(function () {
  ...
  let exportStr = '<span class="breadcrumb-elements-item cursor-pointer" onclick="testFunc()">\n  <i class="icon-download4 mr-2"></i>\n  <span class="">测试</span>\n</span>';
  $('#refreshBoard').before(exportStr);
  $('.sidebar-main-toggle').addClass('demo-chartDraw');
});

function testFunc() {
  // window.parent.document.getElementById('0_1').contentWindow.tableDataDraw('#contractList_table'); //重新加载清单表
  $('#contractList_table tbody').on('click', "tr td:not('.flexCon , .progress_status, .checkPart, .dataTables_empty')", function () {});
}
...
$(function () {
  ...
  let oFrm = document.getElementById('0_1');
  oFrm.onload = function () {
    $('#0_1')[0]
      .contentWindow.$('#contractList_table tbody')
      .on('click', "tr td:not('.flexCon , .progress_status, .checkPart, .dataTables_empty')", function () {
        window.parent.document.getElementById('1_4').contentWindow.tableDataDraw('#contractList_table'); //重新加载清单表
      });
  };
});

function initMap() {
  let dom = $('[data-name="2"]').find('.echart');
  let echartId = dom.attr('id');
  echarts.registerMap('newChina', chinaJson); // 注册地图
    ...
    geo: {
      ...
      itemStyle: {
        normal: {
          ...
          borderColor: '#9e9e9e', // 区域边框颜色
          areaColor: '#f5f5f5', // 区域颜色
        },
        emphasis: {
          areaColor: '#607d8b',
        },
...
function initPie() {
  let dom = $('[data-name="3"]').find('.echart');
  let echartId = dom.attr('id');
  echarts.registerMap('newChina', chinaJson); // 注册地图
