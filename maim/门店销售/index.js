...
function C2R2_Main(_, params) {
  ...
  addBaiDuMap(function () {
    ...
    // 随机向地图添加标注
    let comm = `SELECT s1.CityCN city, count(s1.CityCN) qty, s2.Longitude x, s2.Latitude y FROM app${Userinfo.app}_storelist_actual s1
    ...
    function addMarker(point, cityName) {
      // 创建图标对象
      var myIcon = new BMap.Icon("http://easyview.seepln.com/assets/location_city.png", new BMap.Size(18, 18), {
        anchor: new BMap.Size(9, 18), // 设置图片偏移。
      ...
      marker.addEventListener("click", function () {
        ...
        map.clearOverlays();
        showCityStore(cityName);
      });
      map.addOverlay(marker);
      cfs.card.head.getDom(cardName).find(".customLoader").hide(); //隐藏loading
    }
    function showCityStore(city) {
      let comm = `SELECT StoreNameCN name, Longitude x, Latitude y FROM app${Userinfo.app}_storelist_actual where CityCN = "${city}";`;
      let res = cfs.request.foundation.runComm(comm);
      if (res.err) {
        ForSwal("读取数据失败:" + res.err.Message);
      } else {
        for (let i = 0; i < res.res.length; i++) {
          addSoreMarker(new BMap.Point(res.res[i].x, res.res[i].y), res.res[i].name);
        }
      }
    }
    function addSoreMarker(point, storeName) {
      // 创建图标对象
      var myIcon = new BMap.Icon("http://easyview.seepln.com/assets/location_store.png", new BMap.Size(18, 18), {
        anchor: new BMap.Size(9, 18), // 设置图片偏移。
        imageOffset: new BMap.Size(0, 0), // 设置图片偏移
      });
      // 创建标注对象并添加到地图
    ...
    function showStoreDetail(store) {
      let comm = `SELECT storename 门店名称, sales 销售额, adt 日均交易量,city 城市,adress 地址,tatype 门店类型,openyear 开业年份
      ...
      if (res.err) {
        ...
        let height = "150px";
        let div = $(`<div id="Cus_StoreTable" style="position: absolute; z-index: 1000; height: ${height}; bottom:0;">
                      <div class="card" data-name="R2C2_add1" style="height: ${height};background-color:rgba(255,255,255,0.7)">
        ...
        let table = `
          <table id="table_StoreTable" class="table table-xs table-hover" style="">
            <tr>
              <th scope="col" style="width: 100px;padding: .5rem .5rem;">门店名称</th>
              <th scope="col" style="width: 100px;padding: .5rem .5rem;">城市</th>
              <th scope="col" style="padding: .5rem .5rem;">地址</th>
              <th scope="col" style="width: 100px;padding: .5rem .5rem;">销售额</th>
              <th scope="col" style="width: 110px;padding: .5rem .5rem;">日均交易量</th>
              <th scope="col" style="width: 100px;padding: .5rem .5rem;">门店类型</th>
              <th scope="col" style="width: 100px;padding: .5rem .5rem;">开业年份</th>
            </tr>
            <tbody>
              <tr>
                <td style="padding: .5rem .5rem;">${res.res[0].门店名称}</td>
                <td style="padding: .5rem .5rem;">${res.res[0].城市}</td>
                <td style="padding: .5rem .5rem;">${res.res[0].地址}</td>
                <td style="padding: .5rem .5rem;">${res.res[0].销售额}</td>
                <td style="padding: .5rem .5rem;">${res.res[0].日均交易量}</td>
                <td style="padding: .5rem .5rem;">${res.res[0].门店类型}</td>
                <td style="padding: .5rem .5rem;">${res.res[0].开业年份}</td>
              </tr>
            </tbody>
          </table>
        `;

        $("[data-name=C2R2]").css("position", "relative");

        $("[data-name=C2R2]").append(div).find("#Cus_StoreTable").find("div.card-body").append(table);
