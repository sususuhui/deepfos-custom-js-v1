...
// let extra_Script = document.createElement("script");
// extra_Script.setAttribute("src", "../js/StbDemo/map/js/china.js");
// document.head.appendChild(extra_Script);
...
$(document).ready(function () {
  ...
  // 添加地图后退按钮
  let btn = `<button type="button" class="btn btn-primary legitRipple">后退</button>`;
...
async function mapBack() {
  ...
  if (newOperationArrayLen == 2) {
    ...
    const { provinceDes } = operationArray[1];
    initProvinceMap(provinceDes, resultData, true, true);
  } else if (newOperationArrayLen == 1) {
    initProvinceMap("china", resultData, true, false);
  }
...
async function demoInit() {
  ...
  initProvinceMap("china", provinceData, false, true);
}
...
function combineTableContent(tableData) {
  ...
  if (Form) {
    Form.forEach((valueItem, valueIndex) => {
      ...
      // if (valueIndex != 0) {
      contentStr += `
        <tr mapparam='${AreaAlias || ProvinceAlias || ""}' region='${region}' level='${Level}' Area='${Area || ""}' provinceDes='${Province || ""}' city="${City || ""}" >
          ${headerCodeField.map((v, i) => {
            let tdContent = `<td>${valueItem[v]}</td>`;
            if (v === "Rate") {
              tdContent = `
...
// 初始化地图
function initProvinceMap(mapName, provinceData, setOptionFlag, chartInitFlag, fix = {}) {
  // 获取地图数据key为Mao得值
  ...
  if (mapName === "china") {
    initEchartSMap(mapName, consumeData, achievementRateData, setOptionFlag, chartInitFlag, { zoom: 0.6, center: [103.55, 34.32] });
  } else if (mapName === "Area") {
    initEchartSMap("china", consumeData, achievementRateData, setOptionFlag, chartInitFlag, fix);
  } else {
...
async function chartClick(param) {
  ...
  if (level == "Nation") {
    ...
    combineTableContent(provinceData);
    let fixArea = {
      "T01": { zoom: 0.8, center: [103.55, 40.32] },
      "T02": { zoom: 0.8, center: [113.55, 34.32] },
      "T03": { zoom: 0.8, center: [103.55, 30.32] },
      "T04": { zoom: 0.8, center: [95.55, 34.32] },
    };
    debugger;
    initProvinceMap("Area", provinceData, true, false, fixArea[region]);
  } else if (level == "Area") {
    // 获取省下面的市数据
    ...
    scriptObj.onload = function () {
      initProvinceMap(provinceDes, cityData, true, false);
    };
