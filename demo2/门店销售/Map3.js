...
$(document).ready(function () {
  ...
  LoadBaiduMapScript();
  // 添加地图后退按钮
  let btn = `<button type="button" class="btn btn-primary legitRipple btn-sm">后退</button>`;
  $("[data-name=MapBig]").find(".header-elements").append(btn);
  // 后退按钮绑定事件
  $(".legitRipple").on("click", () => {
    mapBack();
  });
  // 添加表格布局
  $("[data-name=Table]").find(".card-body").css("padding", "0");
  let table = `
  ...
  `;
  $("[data-name=Table]").find(".card-body").append(table);
  const mapDivHeight = $("[data-name=Table]").find(".card-body").height() - 1;
  // 左侧table最大高度
  ...
  <div class="echartWrap">
    <div id="salesOverview" class="bg-white computHeight"></div>
    <div class="row">
      <div class="col-lg-6">
        <div id="charts1">
  ...
  `;
  $("[data-name=MapBig]").find(".card-body").append(map);
  // 设置右侧地图echarts高度
  $("#salesOverview").css({
    height: `${mapDivHeight - 20}px`,
  });
...
function mergeProvinces(chinaJson, names, properties) {
  let initChinaJson = _.cloneDeep(chinaJson);
  //合并大区里省份的coordinates
  let features = initChinaJson.features;
  let polygons = [];
  let polygons2 = [];
  ...
  initChinaJson.features = newFeatures;

  echarts.registerMap("newChina", initChinaJson); // 注册地图
}
 ...
 * @param {*} arr
 */
function initAreaMap(chinaJson, arr) {
  let initAreaJson = _.cloneDeep(chinaJson);

  //合并大区里省份的coordinates
  let features = initAreaJson.features;

  let newFeatures = features;

  let a = newFeatures.filter(function (val, i) {
    return arr.indexOf(val.properties.name) != -1;
  });

  initAreaJson.features = a;

  echarts.registerMap("area", initAreaJson); // 注册地图
}

async function demoInit() {
  const demoData = await getData();
  const resultData = JSON.parse(demoData.result);
  // 生成表格
  combineTableContent(resultData);
  // 生成地图
  initMap("newChina", resultData, false, true, { zoom: 0.5, center: [103.55, 34.32] });
}

// 按钮后退事件
async function mapBack() {
  // 如果已经是中国地图则不做任何处理
  const operationArrayLen = operationArray.length;
  if (operationArrayLen == 1 || !backFlag) {
    return;
  }
  ...
  operationArray.splice(operationArrayLen - 1, 1);

  const newOperationArrayLen = operationArray.length;
  const getDataRes = await getData({ Year: "2020", Period: "5", Region: operationArray[newOperationArrayLen - 1].region, MonthlyStructure: "1", user_id: "123456" });

  const resultData = JSON.parse(getDataRes.result);

  // 渲染左侧表格数据
  combineTableContent(resultData);
  if (newOperationArrayLen == 3) {
    chart.dispose();
    pieShapeChart.dispose();
    ColumnarChart.dispose();
    $("#charts1").height("0px");
    $("#charts2").height("0px");
    $("#salesOverview").css("height", $("#salesOverview").height() + 300 + "px");
    initMap(operationArray[newOperationArrayLen - 1].province, resultData, true, true);
  } else if (newOperationArrayLen == 2) {
    let fixArea = {
      T01: { zoom: 1, center: [115.0, 40.0] },
      T02: { zoom: 1, center: [115.0, 30.0] },
      T03: { zoom: 1, center: [110.0, 23.5] },
      T04: { zoom: 1, center: [95.0, 34.0] },
    };
    initMap("Area", resultData, true, false, fixArea[operationArray[newOperationArrayLen - 1].region]);
...
function combineTableContent(tableData) {
  ...
  if (Form) {
    Form.forEach((valueItem, valueIndex) => {
      const { AreaAlias, AreaCode, ProvinceAlias, ProvinceCode, CityCode, Area, Province, City } = valueItem;
      const region = Level === "Nation" ? AreaCode : Level === "Area" ? ProvinceCode : CityCode;
      contentStr += `
        <tr level='${Level}' region='${region}' area='${Area || ""}' province='${Province || ""}' provinceAlias='${ProvinceAlias || ""}' city="${City || ""}" >
    ...
    .each((i, v) => {
      ...
      trObj.on("click", () => {
        ...
        chartClick({
          ...
          area: trObj.attr("area"),
          province: trObj.attr("province"),
          provinceAlias: trObj.attr("provinceAlias"),
 ...
 * @param {*} param
 */
async function chartClick(param) {
  const { level, region, area, province, provinceAlias, city } = param;
  if (["北京", "天津", "上海", "深圳"].indexOf(province) != -1 && level == "Province") {
    return;
  }
  loadingShow(".tableHeight");
  operationArray.push({
  ...
  if (level == "Nation") {
    // 获取区域下面的省数据
    const provinceDataRes = await getData({ Year: "2020", Period: "5", Region: region, MonthlyStructure: "1", user_id: "123456" });
    const provinceData = JSON.parse(provinceDataRes.result);
    // 重新渲染左侧table数据
    combineTableContent(provinceData);
    let fixArea = {
      T01: { zoom: 1, center: [115.0, 40.0] },
      T02: { zoom: 1, center: [115.0, 30.0] },
      T03: { zoom: 1, center: [110.0, 23.5] },
    ...
    let areaProvince = {
      T01: ["北京", "天津", "河北", "山西", "内蒙古", "黑龙江", "吉林", "辽宁"],
      T02: ["山东", "江苏", "安徽", "江西", "浙江", "福建", "上海", "台湾", "河南", "湖北"],
      T03: ["广东", "广西", "海南", "香港", "澳门", "湖南"],
      T04: ["陕西", "甘肃", "青海", "宁夏", "新疆", "重庆", "四川", "云南", "西藏", "贵州"],
    };
    initAreaMap(chinaJson, areaProvince[region]);

    initMap("Area", provinceData, true, false, fixArea[region]);
  } else if (level == "Area") {
    // 获取省下面的市数据
    const cityDataRes = await getData({ Year: "2020", Period: "5", Region: region, MonthlyStructure: "1", user_id: "123456" });
    const cityData = JSON.parse(cityDataRes.result);
    // 重新渲染左侧table数据
    combineTableContent(cityData);
    // 调用初始化echarts方法
    const head = document.getElementsByTagName("head")[0] || document.documentElement;
    let scriptObj = document.createElement("script");
    scriptObj.setAttribute("type", "text/javascript");
    scriptObj.setAttribute("src", `../js/StbDemo/map/js/province/${provinceAlias}.js`);
    scriptObj.setAttribute("name", name);
    scriptObj.onload = function () {
      initMap(province, cityData, true, false);
    };
    head.appendChild(scriptObj);
  } else if (level == "Province") {
    // 如果是市下面区县级别的table数据
    const countyDataRes = await getData({ Year: "2020", Period: "5", Region: region, MonthlyStructure: "1", user_id: "123456" });
    const countyData = JSON.parse(countyDataRes.result);
    // 重新渲染左侧table数据
    combineTableContent(countyData);

    resolutionArea(city, countyData);
 ...
 * @param {*} [fix={}]
 */
function initMap(mapName, data, setOptionFlag, chartInitFlag, fix = {}) {
  // 获取地图数据key为Mao得值
  ...
  // 组合地图显示数据
  Map.forEach((v) => {
    const { Actual, Rate, Area, Province, City } = v;
    consumeData.push({
      value: Rate,
      name: Area || Province || City,
      level: Level,
      ...v,
      Rate,
    });
  ...
  if (mapName === "china") {
    initEchartSMap("newChina", consumeData, achievementRateData, setOptionFlag, chartInitFlag, { zoom: 0.6, center: [103.55, 34.32] });
  } else if (mapName === "Area") {
    initEchartSMap("area", consumeData, achievementRateData, setOptionFlag, chartInitFlag, fix);
  } else {
    initEchartSMap(mapName, consumeData, achievementRateData, setOptionFlag, chartInitFlag, fix);
  }
}

function initEchartSMap(mapName, consumeData, achievementRateData, setOptionFlag, chartInitFlag, fix = {}) {
  console.log(consumeData, "consumeData");

  let valueData = _.cloneDeep(consumeData).map(function (val) {
    return val.value;
  });
  let minValue = Math.min(...valueData);
  let maxValue = Math.max(...valueData);
  ...
  chart.setOption(option, setOptionFlag);
  chart.off("click");
  chart.on("click", (param) => {
    const {
      data: { level, AreaAlias, AreaCode, ProvinceAlias, ProvinceCode, CityCode, Area, Province, City },
    } = param;
...
function chartsOneInit(countyData) {
  ...
  pieShapeChart = echarts.init(document.getElementById("charts1"));
  pieShapeChart.setOption(option, true);
}

function chartsTwoInit(countyData) {
  ...
  ColumnarChart = echarts.init(document.getElementById("charts2"));
  ColumnarChart.setOption(option, true);
}

/**
 * 解释区县坐标加载百度地图
 */
function resolutionArea(cityName, countyData) {
  ...
  myGeo.getPoint(
    ...
    function (point) {
      if (point) {
        ...
        // 获取百度地图map对象
        const mapObj = chart.getModel().getComponent("bmap").getBMap();

        madeBoundary(mapObj, cityName, countyData);
      } else {
        alert("您选择地址没有解析到结果!");
      }
    },
...
// 区域图
function madeBoundary(map, cityName, countyData) {
  ...
  Map.forEach((areaV) => {
    ...
    bdary.get(County, function (rs) {
      ...
      //添加名称标签层
      let centerlabel = new BMap.Label(County, { offset: new BMap.Size(0, 0) });
