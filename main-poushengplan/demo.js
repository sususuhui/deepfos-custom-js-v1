...
function InitSave() {
  workbook.btn.saveCurrent = async () => {
    ...
    const { checkResult, floatRowTableSaveData } = await workbook.btn.checkBeforeSave(
      "saveCurrent"
    );
    // 如果校验通过则执行保存当前逻辑
    if (checkResult) {
      ...
      const { saveAfterAsyncPython, saveAfterSynchronizePython } =
        workbook.python.getSheetPython(sheetIndex);
      // 设置保存当前时运行脚本的预设参数
      workbook.python.setScriptParameterPythonList(
        [...saveAfterAsyncPython, ...saveAfterSynchronizePython],
        async () => {
          ...
          // 获取保存sheet的单元格批注的结果
          const saveAnnotationResult = await workbook.annotation.saveCellListAnnotation(
            saveAnnotationList
          );
          // 如果存在批注内容
          ...
          const { saveStatus, saveResultList } = await workbook.btn.saveData([
            ...floatRowTableSaveData,
            ...saveDataArr,
          ]);
          // 如果所有sheet都保存成功
          if (saveStatus) {
...
async function handleDirtyCells(sheetInfo, pov_page) {
  ...
  let changedCell = changedAllCell;
    ...
    success: async function (layero, index) {
      ...
      $("#log_jobContentTable").DataTable({
        ...
        language: {
          ...
          info:
            getLanguage("showPaging") +
            " _START_ " +
            getLanguage("to") +
            " _END_ " +
            getLanguage("StudyResults") +
            "，" +
            getLanguage("total") +
            " _TOTAL_ " +
            getLanguage("item"),
