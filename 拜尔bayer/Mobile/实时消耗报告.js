      ...
      series: [
        {
          ...
          data: [
            {
              value: (dataTargetRate * 100).toFixed(2),
              name: dataTargetName,
              title: {
                offsetCenter: ["0%", "100%"],
              },
        ...
        },
        {
          ...
          data: [
            {
              value: (dataForecastRate * 100).toFixed(2),
              name: dataForecastName,
              title: {
                offsetCenter: ["0%", "100%"],
              },
              detail: {
  ...
  let option = {
    ...
    tooltip: {
      ...
      formatter: function (data) {
        ...
        let bodyText = "";
        data.forEach(function (item) {
          bodyText += "<br>" + item.marker + "当日消耗: " + format((item.data / divisor).toFixed(fix));
        });
        return headerText + bodyText;
      },
    },
...
function Chart_R3C2(data, params) {
  ...
  initDataTable("#TableR3C2", $("#chart-" + params.componentId + "").height(), 4);
  foldTable();
  r3c2_status = 1;
...
function foldTable() {
  // 添加树形icon
  $("td.r3c2_treeView").prepend('<i class="icon-arrow-down5 cursor-pointer r3c2_treeView_icon"></i>');
  // 添加树形icon折叠点击事件
  $("i.r3c2_treeView_icon.icon-arrow-down5").click(function () {
    ...
    if ($(this).attr("class").indexOf("icon-arrow-down5") != -1) {
      $(this).attr("class", "icon-arrow-right5 cursor-pointer r3c2_treeView_icon");
      nextFoldTable(currentFoldTableDom, currentIndex);
    } else {
      $(this).attr("class", "icon-arrow-down5 cursor-pointer r3c2_treeView_icon");
  ...
  data.forEach(function (rowItem, x) {
    ...
    rowItem.forEach(function (tdVal, i) {
      ...
      if (i == 0 && cellVal.indexOf("合计") != -1) {
        tBodyHtml += '<td class="r3c2_treeView r3c2_type" style=" text-align:left; ' + '">' + cellVal + "</td>";
      } else if (i == 1 && cellVal.indexOf("合计") != -1) {
        tBodyHtml += '<td class="r3c2_treeView r3c2_regional" style=" text-align:left; ' + '">' + cellVal + "</td>";
      } else if (i == 2 && cellVal.indexOf("合计") != -1) {
        tBodyHtml += '<td class="r3c2_treeView r3c2_provincial" style=" text-align:left; ' + '">' + cellVal + "</td>";
      } else if (i < 3 && cellVal.indexOf("合计") == -1) {
        tBodyHtml += '<td style=" text-align:left;padding-left: 24px; ' + '">' + cellVal + "</td>";
      } else {
        tBodyHtml += "<td>" + cellVal + "</td>";
      }
