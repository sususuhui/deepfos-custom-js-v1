        ...
        children: [
          {
            ...
            style: {
              fill: "#333",
              text: [
                "VS " + pythonData.PriorYear + "年YTD消耗%",
                "VS " + pythonData.CurYear + "年YTD指标%",
                "YTD消耗 VS 全年指标",
                "",
                pythonData.PriorYear + "年消耗",
                pythonData.CurYear + "年指标",
                pythonData.CurYear + "年YTD消耗",
              ].join("\n"),
          ...
          },
          {
            ...
            style: {
              fill: "#333",
              text: [pythonData.CY_PY + "%", pythonData.CY_Target + "%", pythonData.YTD_Target + "%", "", pythonData.PY_Actual + "%", pythonData.CY_Target + "%", pythonData.CY_Actual + "%"].join(
                "\n"
              ),
          ...
          },
          {
            ...
            style: {
              fill: "#333",
              text: [pythonData.CurYear + "年YTD消耗", "", "YTD消耗进度", "", "YTD消耗阶段", "", ""].join("\n"),
              font: "bold 15px Microsoft YaHei",
              textAlign: "right",
            },
          },
  ...
  option = {
    ...
    series: [
      ...
      },
      {
        ...
        itemStyle: {
          color: pythonData.Ach110.color,
        },
        data: pythonData.Ach110.data,
      },
      {
        xAxisIndex: 1,
        yAxisIndex: 1,
        ...
        itemStyle: {
          color: pythonData.Ach99.color,
        },
        data: pythonData.Ach99.data,
      },
      {
        xAxisIndex: 1,
        yAxisIndex: 1,
...
function addDiv_R2C2(componentId, upDivHeight, downDivHeight, divWidth) {
  let html = `
  <div id="chart-${componentId}-newDiv">
    <div id="chart-${componentId}-upDiv" style="height: ${upDivHeight}px; width: ${divWidth}px"></div>
    <div id="chart-${componentId}-downDiv" style="height: ${downDivHeight}px; width: ${divWidth}px"></div>
  </div>
  `;
      ...
      $.ajax(settings).done(function (response) {
        response.resultList.forEach(function (data, i) {
          if (data.description_1 == domSelected) {
            ...
            var dom = $('[data-name="Monthly_RootData"]').find(".elementIframe")[0].contentWindow;
            resetColumn(dom);
            if (data.level == 3) {
