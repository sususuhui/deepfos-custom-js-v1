...
function r1c2(data, params) {
  ...
  r1c2_data.eff_total.forEach(function (val) {
    ...
    <div>
      <span class="font-size-sm opacity-75 mr-2">${val[0]} ${yearSuffix[parseInt(val[0])]}</span>
      <span class="oldIncome1" style="font-size: 1.125rem;">${_.isNumber(val[1]) ? val[1].toFixed(2) : val[1]}</span>
    </div>
    `;
    $("#" + id)
      .find("#total")
...
function r4(data, params) {
  ...
  let option = {
    ...
    legend: {
      data: ["Revenue_Existing Leases", "Revenue_New&Renewal Leases", "Avg. Occ%", `Revenue_Yr${getCurrentYear(true)} Budget`],
      orient: "vertical",
      left: "center",
      bottom: "bottom",
    },
    ...
    series: [
      ...
      {
        name: "Revenue_New&Renewal Leases",
        type: "bar",
        stack: "bar",
        barMaxWidth: 30,
        color: "#82bc00",
...
<div class="dropdown-menu dropdown-menu-left">
  ...
  let switchDetail = `<div class="btn-group mr-2" style="position: absolute;left: 5%;top: 1%;z-index: 99;">
    ...
    <button type="button" class="btn btn-light" onclick='switchKey1_r5()' style="display:none">NOI</button>
    <button type="button" class="btn btn-light" onclick='switchKey2_r5()' style="display:none">NOI Margin</button>
    <button type="button" class="btn btn-light" onclick='switchKey3_r5()' style="display:none">Opex&repair Cost</button>
    <button type="button" class="btn btn-light" onclick='switchKey4_r5()' style="display:none">PM Fee</button>
    <button type="button" class="btn btn-light" onclick='switchKey5_r5()' style="display:none">Tax</button>
  </div>`;
...
function switchPercentage_r5() {
    ...
    if (x == 0) {
      trV.forEach((tdV, y) => {
        if ([7, 9, 11].includes(y) && parseFloat(tdV) < 0) {
          ...
          tbodyHtml += `<td class="text-left split-line" style="font-weight:bold">${tdV}</td>`;
      ...
      trV.forEach((tdV, y) => {
        if ([7, 9, 11].includes(y) && parseFloat(tdV) < 0) {
          ...
          tbodyHtml += `<td class="text-left split-line" >${tdV}</td>`;
        } else if ([12, 17, 21].includes(y)) {
          tbodyHtml += `<td class="split-line">${tdV}</td>`;
        } else {
          let classAttr = "";
  ...
  $(`${"#" + r5_id} #total-table-part`)
    .find("tbody")
    .html(tbodyHtml);
  // initDataTable(`${"#" + r5_id} #total-table-part`, 1300, 1);
...
<button type="button" class="btn btn-light" onclick='switchKey1_r6()' style="display:none">Revenue excl leveling</button>
<button type="button" class="btn btn-light" onclick='switchKey2_r6()' style="display:none">Avg. Occ%</button>
<button type="button" class="btn btn-light" onclick='switchKey3_r6()' style="display:none">Avg. Effective Rent</button>
<button type="button" class="btn btn-light" onclick='switchKey4_r6()' style="display:none">NLA</button>
<button type="button" class="btn btn-light" onclick='switchKey5_r6()' style="display:none">Leveling</button>
</div>`;
...
function switchPercentage_r6() {
  ...
  $(`${"#" + r6_id} #total-table-part`)
    .find("tbody")
    .html(tbodyHtml);
  // initDataTable(`${"#" + r6_id} #total-table-part`, 1300, 1);
